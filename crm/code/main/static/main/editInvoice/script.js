...
let taskFunctions = {
    ...
    // Take initial snapshot of current tasks and save to masterList
    takeSnapshot() {
        let tasks = document.querySelectorAll(".content__task");
        let snapshot = [];
        tasks.forEach((task) => {
            let qty = task.querySelector(".content__task-qty").innerText;
            let price = task.querySelector(".content__task-price").innerText;
            snapshot.push(
                {
                    num: task.querySelector(".content__task-num").innerText,
                    name: task.querySelector(".content__task-name").innerText,
                    qty: qty,
                    price: price,
...
let invoiceFunctions = {
    ...
    save() {
        ...
        // Send AJAX 
        fetch(`/invoices/save/${INVOICE_ID}`, {
            method: 'POST',
            headers: {
...
window.addEventListener("load", (loadEvent) => {
    ...
    document.querySelector(".delete-invoice-btn").addEventListener("click", handlers.deleteInvoicePrompt);

    // Cancelling toggles prompt visibility, other button deletes invoice
    let deleteInvoicePrompt = document.querySelector(".delete-invoice-prompt");
    deleteInvoicePrompt.querySelectorAll("button")[1].addEventListener("click", handlers.deleteInvoicePrompt);
    deleteInvoicePrompt.querySelectorAll("button")[0].addEventListener("click", handlers.deleteInvoice);
});