...
let searchFunctions = {
    ...
    update(search_results) {
        ...
        for (let job of search_results) {
            table_body.appendChild(tableFunctions.createNewRow(job));
        }
...
let tableFunctions = {
    ...
    attachClickHandlers() {
        document.querySelectorAll("tbody").forEach((elem) => {
            elem.addEventListener("click", handlers.viewJob);
        });
    }
...
let handlers = {
    ...
    //click table row, link to corresponding job
    viewJob(e) {
...
window.onload = () => {
    ...
    // Clicking table row, link to job page
    document.querySelectorAll("tbody").forEach((elem) => {
        elem.addEventListener("click", handlers.viewJob);
    })
