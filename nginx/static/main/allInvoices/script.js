...
let searchFunctions = {
    ...
    update(search_results) {
        ...
        for (let invoice of search_results) {
            table_body.appendChild(tableFunctions.createNewRow(invoice));
        }
...
let handlers = {
    ...
    //click table row, link to corresponding invoice
    viewInvoice(e) {
...
window.onload = () => {
    ...
    // Clicking table row, link to invoice page
    document.querySelectorAll("tbody").forEach((elem) => {
        elem.addEventListener("click", handlers.viewInvoice);
    })
}